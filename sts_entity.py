...
class InputExample(object):
  ...
  def __init__(self, guid, text_a, text_b=None, label=None,entity_a=None,entity_b=None):
...
class InputFeatures(object):
  ...
  def __init__(self, input_ids, input_mask, segment_ids, entity_ids, label_value,char_ids,entity_a,entity_b):
...
class TextsimProcessor(DataProcessor):
  ...
  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "train.out")), "dev")
...
def create_model(bert_config, is_training, input_ids, char_ids, input_mask, segment_ids, entity_ids,
                 labels, num_labels, use_one_hot_embeddings,entity_a,entity_b):
...
def main(_):
  ...
  if FLAGS.do_eval:
    ...
    # print('预测结果为：',results)
    gold = []
    with open(os.path.join(FLAGS.data_dir, "train.out"), 'r') as f:
      lines = f.readlines()
      for line in lines:
