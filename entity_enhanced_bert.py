...
class InputFeatures(object):
  ...
  def __init__(self, input_ids, input_mask, segment_ids, entity_ids, label_value,hand_fea,char_ids,entity_a,entity_b,entity_a_kb,entity_b_kb,similarity):
    ...
    for idx in range(len(lines)):
      ...
      examples.append(
          InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label, hand_fea=None,\
                        ...
                        entity_a_kb=None, entity_b_kb=None))
    return examples
...
def create_model(bert_config, is_training, input_ids, char_ids, input_mask, segment_ids, entity_ids,
                 labels, similarity, num_labels, use_one_hot_embeddings,hand_fea,entity_a,entity_b,\
...
# get the encoded rep from ABCNN
  ...
  with tf.variable_scope("loss"):  
    if is_training == True:
      lstm_output = tf.nn.dropout(output_layer, 0.8)
...
#     entity_b_kb = features["entity_b_kb"]
    ...
    total_loss, pred= create_model(
        ...
        num_labels, use_one_hot_embeddings,hand_fea=None,entity_a=entity_a,entity_b=entity_b,entity_a_kb=None,entity_b_kb=None)
