...
class DataProcessor(object):
  ...
  def _read_fold_data(cls, data_file, fold_file, fold):
    ...
    with tf.gfile.Open(fold_file, "r") as f:
      ...
      train_idx, test_idx = data_split
    return contents, train_idx, test_idx
...
class TextsimProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir, fold):
    """See base class."""
    lines, train_idx, test_idx = self._read_fold_data(os.path.join(data_dir, "all_kg.txt"), os.path.join(data_dir, "train_test.txt"), fold)
    return self._create_examples(lines, train_idx, test_idx, "train")
 
  def get_dev_examples(self, data_dir, fold):
    """See base class."""
    lines, train_idx, test_idx = self._read_fold_data(os.path.join(data_dir, "all_kg.txt"), os.path.join(data_dir, "train_test.txt"), fold)
    return self._create_examples(lines, train_idx, test_idx, "dev")
    # return self._create_examples(
    #     self._read_fold_data(os.path.join(data_dir, "all.json"), os.path.join(data_dir, "train_test.txt"), fold),"dev")
...
def create_model(bert_config, is_training, input_ids, char_ids, input_mask, segment_ids, entity_ids,
  ...
  model = modeling_wo_entity.BertModel(
      config=bert_config,
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate, other_learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if init_checkpoint:
      ...
      ) = modeling_wo_entity.get_assignment_map_from_checkpoint(tvars, init_checkpoint)
      if use_tpu:
...
def main(_):
  ...
  bert_config = modeling_wo_entity.BertConfig.from_json_file(FLAGS.bert_config_file)
  ...
  if FLAGS.do_eval:
    ...
    with open(os.path.join(FLAGS.data_dir, "train_test.txt"), 'r') as f:
      lines = f.readlines()
