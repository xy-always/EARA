...
class TextsimProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir, retrieve_dir):
    ...
    retrives = self._read_retriev_data(os.path.join(retrieve_dir, "train_neighbor.txt"))
    return self._create_examples(lines, retrives, "train")
 
  def get_dev_examples(self, data_dir, retrieve_dir):
    """See base class."""
    lines = self._read_sts_data(os.path.join(data_dir, "dev_kg.txt"))
    retrives = self._read_retriev_data(os.path.join(retrieve_dir, "dev_neighbor.txt"))
  ...
  def get_test_examples(self, data_dir, retrieve_dir):
    ...
    retrives = self._read_retriev_data(os.path.join(retrieve_dir, "test_neighbor.txt"))
    # lines = self._read_sts_data("/disk2/xy_disk2/thesis/STS/LAP/data/BIOSSES/all_kg.txt")
    # retrives = self._read_retriev_data("/disk2/xy_disk2/thesis/STS/RetroSTS/data/BIOSSES/all_neighbor.txt")
...
# get the encoded rep from ABCNN
  ...
  #   #loss = tf.losses.huber_loss(labels,logits)
    ...
    return loss+0.3*aug_loss,output_layer
...
def main(_):
  ...
  if FLAGS.do_train:
    ...
    # train_examples = random.sample(train_examples, 600)
  ...
  if FLAGS.do_eval:
    ...
    # print('预测结果为：',results)
    gold = []
    with open(os.path.join(FLAGS.data_dir, "dev_kg.txt"), 'r') as f:
      lines = f.readlines()
      for line in lines:
  ...
  if FLAGS.do_predict:
    ...
    np.save('n2c2_biobert_kg.npz', result)
    '''
    result = list(result)
    result_xy = []
    for r in result:
    ...
    # print('预测结果为：',result)
      ...
      # with open('/disk2/xy_disk2/thesis/STS/LAP/data/BIOSSES/all_kg.txt', 'r') as f1:
        lines = f1.readlines()
        for line in lines:
