...
class ABCNN():
    def __init__(self, input_a, input_b, labels, s=380, w=3, l2_reg=0.001, model_type="BCNN",  d0=50, di=50, num_classes=1, num_layers=2):
        """
        ...
        # tf.assign(self.x1,input_a)
        # self.x2 = tf.placeholder(tf.float32, shape=[None, d0, s], name="x2")
        # tf.assign(self.x2,input_b)
        # self.y = tf.placeholder(tf.float32, shape=[None, ], name="y")
        # tf.assign(self.y, labels)

        self.x1 = input_a
        self.x2 = input_b
        self.y  = labels
        self.features = tf.placeholder(tf.float32, shape=[None, 2], name="features")
        ...
        #     return np.max(tf.abs(v1-v2), axis=1)

        # def spearman_coeffcient(v1,v2):
        #     rho, p_val = spearmanr(v1,v2)
        #     return rho

        # def kendalltau_coefficient(v1,v2):
            ...
            with tf.name_scope(name_scope + "-conv"):
                with tf.variable_scope("conv") as scope:
                    conv = tf.contrib.layers.conv2d(
                        ...
                        biases_initializer=tf.constant_initializer(1e-03),
                        reuse=reuse,
                        trainable=True,
                        scope=scope
                    )
        ...
        # print('LO_1',LO_1)
        # print('R0_1',RO_1)
        # print(tf.subtract(LO_1,RO_1))
        # print(tf.matmul(tf.transpose(LO_1),RO_1))
        # tf.matmul(tf.transpose(LO_1),RO_1)
        ...
        self.encode_rep = tf.concat([tf.stack(sims, axis=1),tf.subtract(LO_1,RO_1), tf.subtract(LO_2, RO_2)],axis=-1)

            # sims.append(manhantan_diatance(LO_2, RO_2))
        # with tf.variable_scope("output-layer"):
        #     #self.features = tf.reshape(self.features, [-1, 1])
        #     self.output_features = tf.nn.tanh(tf.concat([tf.stack(sims, axis=1)], axis=1, name="output_features"))
        ...
        # self.y_reshape = tf.reshape(self.y,[-1,1])
        # self.prediction = tf.reshape(self.prediction,[-1,1])

        # self.cost = tf.losses.mean_squared_error(self.y_reshape, self.prediction)
    ...
    def get_encode_rep(self):
        return self.encode_rep
