...
import joblib
from get_similarity import external_similarity, get_sim, random_mask
...
class DataProcessor(object):
  ...
  def _read_fold_data(cls, data_file, fold_file, fold):
    ...
    with tf.gfile.Open(fold_file, "r") as f:
      ...
      train_idx, dev_idx = data_split
    return contents, train_idx, dev_idx
...
class TextsimProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir, fold):
    """See base class."""
    lines, train_idx, dev_idx = self._read_fold_data(os.path.join(data_dir, "train_kg.txt"), os.path.join(data_dir, "train_test_82_layer.txt"), fold)
    return self._create_examples(lines, train_idx, dev_idx, "train")
  ...
  def get_dev_examples(self, data_dir, fold):
    ...
    # return self._create_examples(
    #     self._read_fold_data(os.path.join(data_dir, "all.json"), os.path.join(data_dir, "train_test_82_layer.txt"), fold),"dev")

  def get_test_examples(self, data_dir, fold):
    """See base class."""
    lines, train_idx, dev_idx = self._read_fold_data(os.path.join(data_dir, "test_kg.txt"), os.path.join(data_dir, "train_test_82_layer.txt"), fold)
    return self._create_examples(lines, train_idx, dev_idx, "test")
    # return self._create_examples(
    #     self._read_fold_data(os.path.join(data_dir, "all.json"), os.path.join(data_dir, "train_test_82_layer.txt"), fold), "test")

  def get_labels(self):
    """See base class."""
    return ["0", "1"]

  def _create_examples(self, lines, train_idx, dev_idx, set_type):
    """Creates examples for the training and dev sets."""
    examples = []
    if set_type == 'train':
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  entity2id = {'[CLS]':0, 'O': 1, 'SignSymptomMention': 2, 'Predicate': 3, 'DiseaseDisorderMention': 4, 
  ...
  entity_a = random_mask(entity_a, FLAGS.ratio)
  entity_b = random_mask(entity_b, FLAGS.ratio)
...
# get the encoded rep from ABCNN
  with tf.variable_scope('abcnn_embedding'):
    abcnn_embedding = tf.get_variable("abcnn_embedding", shape=[10, 50],
  ...
  ## raw bert + entity KB
  output_layer = tf.tanh(tf.concat([output_layer, encode_rep],axis=-1))
...
def main(_):
  ...
  if FLAGS.do_eval:
    ...
    # print('预测结果为：',results)
    gold = []
    with open(os.path.join(FLAGS.data_dir, "train_kg.txt"), 'r') as f:
      lines = f.readlines()
      contents = []
      for i, line in enumerate(lines):
        content = line.strip().split('\t')
    ...
    with open(os.path.join(FLAGS.data_dir, "train_test_82_layer.txt"), 'r') as f:
      lines = f.readlines()
