...
class InputExample(object):
  ...
  def __init__(self, guid, text_a, text_b=None, label=None,hand_fea=None,entity_a=None,entity_b=None,entity_a_kb=None,entity_b_kb=None):
...
class InputFeatures(object):
  ...
  def __init__(self, input_ids, input_mask, segment_ids, entity_ids, label_value,hand_fea,char_ids,entity_a,entity_b,entity_a_kb,entity_b_kb):
...
class TextsimProcessor(DataProcessor):
  ...
  def _create_examples(self, lines, set_type):
    ...
    if set_type == "dev":
      ...
      with open(os.path.join(FLAGS.data_dir, 'dev_entity.out'),'r') as f:
        abcnn_lines = f.readlines()
        abcnn_input_a_lines = []
        abcnn_input_b_lines = []
        for line in abcnn_lines:
    ...
    for (i, line) in enumerate(lines):
      ...
      entity_a_kb_i = np.array(entity_a_kb[i])
      entity_b_kb_i = np.array(entity_b_kb[i])
      ...
      examples.append(
          InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label, hand_fea=hand_fea,\
                        ...
                        entity_a_kb=entity_a_kb_i, entity_b_kb=entity_b_kb_i))
    return examples
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  entity_ids =[0] + entity_a_ids + [0] + entity_b_ids + [0]
  for token in to_a:
    tokens.append(token)
    segment_ids.append(0)
...
def create_model(bert_config, is_training, input_ids, char_ids, input_mask, segment_ids, entity_ids,
  ...
  '''
  ##TODO
  # get the encoded rep from KB
  with tf.variable_scope('entity_embedding'):

    entity_index_mul = tf.multiply(entity_a_kb, entity_b_kb)
    entity_index_sub = tf.subtract(entity_a_kb, entity_b_kb)

    entity_kb_rep = tf.concat([entity_index_mul, entity_index_sub], axis=-1)
    # entity_rep = entity_index
    print('%%'*20)
    print(entity_kb_rep.shape)
...
# get the encoded rep from ABCNN
  ...
  with tf.variable_scope('char_CNN'):
      cnn_embed_char = tf.layers.Conv1D(filters=FLAGS.char_embed_dim,kernel_size=3,padding='same',activation='tanh',strides=3)(embed_char)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate, other_learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    total_loss, pred= create_model(
        bert_config, is_training, input_ids, char_ids, input_mask, segment_ids, entity_ids, label_values,
        num_labels, use_one_hot_embeddings,hand_fea,entity_a,entity_b,entity_a_kb,entity_b_kb)
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    # print('预测结果为：',result)
      gold = []
      with open('../processed_data/test_data/test.out','r') as f1:
        lines = f1.readlines()
        for line in lines:
