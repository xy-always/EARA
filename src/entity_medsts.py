...
import joblib
import random
from get_similarity import external_similarity, get_sim, get_cls_unsimilarity
...
class InputFeatures(object):
  ...
  def __init__(self, input_ids, input_mask, segment_ids, entity_ids, label_value,char_ids,entity_a,entity_b,similarity,unsimilarity):
    ...
    self.unsimilarity = unsimilarity
...
class TextsimProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir, fold):
    """See base class."""
    lines = self._read_sts_data(os.path.join(data_dir, "train_kg.txt"))
    return self._create_examples(lines, "train")
 
  def get_dev_examples(self, data_dir, fold):
    """See base class."""
    lines = self._read_sts_data(os.path.join(data_dir, "test_kg.txt"))
  ...
  def get_test_examples(self, data_dir, fold):
    ...
    # return self._create_examples(
    #     self._read_fold_data(os.path.join(data_dir, "all.json"), os.path.join(data_dir, "train_test.txt"), fold), "test")
...
#   entity_ids =[0] + entity_a_ids + [0] + entity_b_ids + [0] # before revise: [SEP]需要注意
  entity_ids =[0] + entity_a_ids + [1] + entity_b_ids + [1] # after revise: [SEP]不需要注意
  for token in to_a:
    tokens.append(token)
    segment_ids.append(0)
  ...
  cls_un_simlarity = get_cls_unsimilarity(entity_ids)
  ...
  assert len(cls_un_simlarity) == max_seq_length
  ...
  if ex_index < 5:
    ...
    tf.logging.info("unsimilarity: %s" % " ".join([str(x) for x in cls_un_simlarity]))
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    features["unsimilarity"] = create_float_feature(feature.unsimilarity)
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  name_to_features = {
      ...
      "unsimilarity": tf.FixedLenFeature([seq_length], tf.float32),
...
def create_model(bert_config, is_training, input_ids, char_ids, input_mask, segment_ids, entity_ids,
                 labels, similarity, unsimilarity, num_labels, use_one_hot_embeddings,entity_a,entity_b):
  ...
  input_mask_ = tf.cast(input_mask, dtype=tf.float32)
  unsimilarity_atten = tf.expand_dims(unsimilarity * input_mask_, axis=-1)
  mask_output = all_output * unsimilarity_atten
  unsimilarity_pre = tf.reduce_sum(all_output * unsimilarity_atten, axis=1) / tf.reduce_sum(all_output * unsimilarity_atten, axis=1)
  print('unsimilarity attention')
  print(unsimilarity_pre.shape)
  
  output_layer = output_layer + 0.2 * unsimilarity_pre
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate, other_learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    unsimilarity = features["unsimilarity"]

    is_training = (mode == tf.estimator.ModeKeys.TRAIN)

    total_loss, pred= create_model(
        bert_config, is_training, input_ids, char_ids, input_mask, segment_ids, entity_ids, label_values, similarity, unsimilarity,
...
def main(_):
  ...
  if FLAGS.do_eval:
    ...
    # print('预测结果为：',results)
    gold = []
    with open(os.path.join(FLAGS.data_dir, "test_kg.txt"), 'r') as f:
      lines = f.readlines()
      for line in lines:
  ...
  if FLAGS.do_predict:
    ...
    # print('预测结果为：',result)
      gold = []
      with open(os.path.join(FLAGS.data_dir, "test_kg.txt"), 'r') as f1:
        lines = f1.readlines()
        for line in lines:
