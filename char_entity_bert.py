...
class InputExample(object):
  ...
  def __init__(self, guid, text_a, text_b=None, label=None,hand_fea=None,entity_a=None,entity_b=None):
...
class InputFeatures(object):
  ...
  def __init__(self, input_ids, input_mask, segment_ids, label_value,hand_fea,char_ids,entity_a,entity_b):
...
class TextsimProcessor(DataProcessor):
  ...
  def _create_examples(self, lines, set_type):
    ...
    if set_type == "train":
      hand_feas = joblib.load(os.path.join(FLAGS.data_dir, 'pkl/train_feature.pkl'))
      entity_a_kb = joblib.load(os.path.join(FLAGS.data_dir, 'out1_train.pkl'))
      entity_b_kb = joblib.load(os.path.join(FLAGS.data_dir, 'out2_train.pkl'))
    if set_type == "dev":
      hand_feas = joblib.load(os.path.join(FLAGS.data_dir, 'pkl/dev_feature.pkl'))
      entity_a_kb = joblib.load(os.path.join(FLAGS.data_dir, 'out1_dev.pkl'))
      entity_b_kb = joblib.load(os.path.join(FLAGS.data_dir, 'out2_dev.pkl'))
    if set_type == "test":
      hand_feas = joblib.load('../processed_data/test_data/pkl/test_feature.pkl')
      entity_a_kb = joblib.load('../processed_data/test_data/out1_test.pkl')
      entity_b_kb = joblib.load('../processed_data/test_data/out2_test.pkl')
    ...
    for (i, line) in enumerate(lines):
      ...
      entity_a = np.array(entity_a_kb[i])
      entity_b = np.array(entity_b_kb[i])

      ##TODO
      hand_fea = np.array(hand_feas[i])
      ...
      examples.append(
          InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label, hand_fea=hand_fea,
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  entity2id = joblib.load('entity2id.pkl')

  ##TODO
  hand_fea = [i for i in example.hand_fea]
  ...
  assert len(example.entity_a) == FLAGS.entity_vec
  assert len(example.entity_b) == FLAGS.entity_vec
  ...
  if ex_index < 5:
    ...
    tf.logging.info("entity_a: %s" % " ".join([str(x) for x in example.entity_a]))
    tf.logging.info("entity_b: %s" % " ".join([str(x) for x in example.entity_b]))
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    features["entity_a"] = create_float_feature(feature.entity_a)
    features["entity_b"] = create_float_feature(feature.entity_b)
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  name_to_features = {
      ...
      "entity_a": tf.FixedLenFeature([FLAGS.entity_vec], tf.float32),
      "entity_b": tf.FixedLenFeature([FLAGS.entity_vec], tf.float32)
  }
...
def create_model(bert_config, is_training, input_ids, char_ids, input_mask, segment_ids,
                 labels, num_labels, use_one_hot_embeddings,hand_fea,entity_a,entity_b):
  ...
  with tf.variable_scope('entity_embedding'):

    entity_index_mul = tf.multiply(entity_a, entity_b)
    entity_index_sub = tf.subtract(entity_a, entity_b)

    entity_rep = tf.concat([entity_index_mul, entity_index_sub], axis=-1)
    # entity_rep = entity_index
  ...
  # output_layer = tf.concat([output_layer, entity_rep],axis=-1)   
  ...
  with tf.variable_scope('char_CNN'):
      cnn_embed_char = tf.layers.Conv1D(filters=FLAGS.char_embed_dim,kernel_size=3,padding='same',activation='tanh',strides=1)(embed_char)
      ...
      print(char_avg_rep)
      # sys.exit(0)
  # output_layer = tf.concat([output_layer, tf.reshape(char_max_rep,[-1,FLAGS.char_embed_dim]), encode_rep],axis=-1)
  # output_layer = tf.concat([output_layer, tf.nn.tanh(entity_rep)],axis=-1)
  # output_layer = tf.concat([output_layer,  tf.reshape(char_max_rep,[-1,FLAGS.char_embed_dim]),  tf.reshape(char_avg_rep,[-1,FLAGS.char_embed_dim])],axis=-1)
  ...
  #     "output_weights_11", [hidden_size//2, hidden_size],
  #     initializer=tf.truncated_normal_initializer(stddev=0.02))
  # output_bias_1 = tf.get_variable(
  #     "output_bias_11", [hidden_size//2], initializer=tf.zeros_initializer())
  # output_weights_2 = tf.get_variable(
  #     "output_weights_21", [1, hidden_size//2],
  #     initializer=tf.truncated_normal_initializer(stddev=0.02))
  # output_bias_2 = tf.get_variable(
  #     "output_bias_21", [1], initializer=tf.zeros_initializer())
  ...
  # hidden_size = output_layer.shape[-1].value
  # output_weights_1 = tf.get_variable(
  #     "output_weights_1", [hidden_size/2, hidden_size],
  #     initializer=tf.truncated_normal_initializer(stddev=0.02))
  # output_bias_1 = tf.get_variable(
  #     "output_bias_1", [hidden_size/2], initializer=tf.zeros_initializer())
  # output_weights_2 = tf.get_variable(
  #     "output_weights_2", [1, hidden_size/2],
  #     initializer=tf.truncated_normal_initializer(stddev=0.02))
  # output_bias_2 = tf.get_variable(
  #     "output_bias_2", [1], initializer=tf.zeros_initializer())
  ...
  ml_rep_w = tf.get_variable(
      "ml_rep_w", [entity_rep.shape[-1].value,70],
      initializer=tf.truncated_normal_initializer(stddev=0.02))

  # bert_rep_b = tf.get_variable('bert_rep_b',shape=[100], initializer=tf.zeros_initializer())
  ml_rep_b = tf.get_variable('ml_rep_b',shape=[70], initializer=tf.zeros_initializer())

  # bert_rep = tf.nn.tanh(tf.matmul(output_layer,bert_rep_w)+bert_rep_b)
  ml_rep = tf.nn.tanh(tf.matmul(entity_rep,ml_rep_w)+ml_rep_b)
  ...
  # ##gate 
  # alpha = sigmoid(W[h:x])
  # rep = alpha*bert_rep + (1-alpha)*ml_rep
  # W_gate = tf.get_variable('W_gate',shape=[200,100])
  # alpha = tf.nn.sigmoid(tf.matmul(tf.concat([bert_rep,ml_rep],axis=-1),W_gate))

  # rep = tf.concat([tf.multiply(alpha,bert_rep),tf.multiply((1-alpha),ml_rep)],axis=-1)

  # print('=============rep shape==================')
  # print(rep.shape)
  # output_layer = tf.concat([output_layer, ml_rep], axis=-1)
  # output_layer = tf.tanh(tf.concat([output_layer,  ml_rep, tf.reshape(char_avg_rep,[-1,FLAGS.char_embed_dim]), tf.reshape(char_max_rep,[-1,FLAGS.char_embed_dim])],axis=-1))
  output_layer = tf.tanh(tf.concat([output_layer, tf.reshape(char_avg_rep,[-1,FLAGS.char_embed_dim])],axis=-1))
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate, other_learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    total_loss , pred= create_model(
        bert_config, is_training, input_ids, char_ids, input_mask, segment_ids, label_values,
        num_labels, use_one_hot_embeddings,hand_fea,entity_a,entity_b)
...
def main(_):
  ...
  if FLAGS.do_eval:
    ...
    # print('预测结果为：',results)
    gold = []
    with open(os.path.join(FLAGS.data_dir, "dev.out"), 'r') as f:
      lines = f.readlines()
      for line in lines:
  ...
  if FLAGS.do_predict:
    ...
    # print('预测结果为：',result)
    gold = []
    with open('../processed_data/test_data/test.out','r') as f:
      lines = f.readlines()
      for line in lines:
        tmp = float(line.strip('\n').split('\t')[0])
        gold.append(tmp)

    test_pearson = np.corrcoef(np.array(result_xy),np.array(gold))[0][1]
    print('验证集皮尔逊系数为: ',test_pearson)
    # print(os.path.join(FLAGS.output_dir, "_result.txt"))
    # with open(FLAGS.output_dir+"_result.txt",'a') as f:
    #   f.write(FLAGS.output_dir)
    #   f.write('\t')
    #   f.write(str(test_pearson))
    #   f.write('\n')
    # sys.exit(0)
    # print('gold label：',gold)

    # test_pearson = np.corrcoef(result,gold)[0][1]
